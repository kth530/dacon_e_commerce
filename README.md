...
Tableau 대시보드 제작을 위한 데이터 소스 구성 코드를 별도로 정리했다. 대시보드 산출용 CSV는 로컬 작업 산출물로 관리하며, 공개 저장소에는 코드 중심으로 남긴다.

| 파일 | 역할 |
|------|------|
| [`tableau/tableau_views.sql`](tableau/tableau_views.sql) | Tableau용 고객·주문·월별 뷰 정의 |
| [`tableau/export_csv.py`](tableau/export_csv.py) | Tableau 입력용 CSV 추출 스크립트 |
