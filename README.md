...
분석 결과를 `매출·리텐션`, `등급·세그먼트`, `액션·실험`의 3개 화면으로 구성했다. 매출 구조와 코호트 리텐션을 확인한 뒤 등급·세그먼트별 우선순위를 비교하고, 관찰 결과를 고객군별 실험 후보로 연결한다.
...
- 카테고리 매출 구성을 선택하면 해당 카테고리의 매출 상위 제품 5개가 갱신된다.
- 등급·세그먼트 선택을 통해 고객 가치와 행동 차이를 비교하고 실험 대상을 확인한다.
